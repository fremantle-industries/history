defmodule History.FundingRates.DownloadChunksBroadway do
  use Broadway
  require Logger
  alias Broadway.Message
  alias History.{FundingRateHistoryChunks, FundingRateHistoryJobs, FundingRates}

  @default_concurrency 2

  @spec start_link(term) :: Supervisor.on_start()
  def start_link(_) do
    processor_concurrency = Application.get_env(:history, :download_funding_rate_concurrency, @default_concurrency)

    Broadway.start_link(__MODULE__,
      name: __MODULE__,
      producer: [
        module: {FundingRates.DownloadChunksProducer, []},
  ...
  def ack(:ack_id, successful, failed) do
    ...
    |> Enum.map(fn m ->
      ...
      {:ok, _} = FundingRateHistoryChunks.update(chunk, %{status: new_status})
    ...
    |> Enum.each(fn job_id ->
      ...
      if total_chunks == total_complete + total_error + total_not_found do
        ...
        FundingRates.PubSub.broadcast_update(job.id, job_status)
      end
    end)
