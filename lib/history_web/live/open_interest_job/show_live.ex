defmodule HistoryWeb.OpenInterestJob.ShowLive do
  use HistoryWeb, :live_view
  alias History.{OpenInterestHistoryJobs, OpenInterestHistoryChunks}

  @impl true
  def mount(%{"id" => id}, _session, socket) do
    OpenInterestHistoryJobs.subscribe_by_id(id)
    OpenInterestHistoryChunks.subscribe_by_job_id(id)
  ...
  @impl true
  def handle_info({"open_interest_history_job:" <> _, _}, socket) do
    socket =
      socket
      |> assign_job()
      |> assign_chunk_status_totals()
  ...
  @impl true
  def handle_info({"open_interest_history_chunk:job:" <> _, _}, socket) do
    socket =
      socket
      |> assign_chunk_status_totals()
      |> assign_chunks()
  ...
  defp assign_job(socket) do
    socket
    |> assign(job: OpenInterestHistoryJobs.get!(socket.assigns.job_id))
  end
  ...
  defp assign_chunks(socket) do
    ...
    |> assign(
      :chunks,
      OpenInterestHistoryChunks.by_job(job.id,
        page: current_page,
  ...
  defp assign_chunk_status_totals(socket) do
    ...
    |> Enum.reduce(
      ...
      fn {status, _color}, socket ->
        total = OpenInterestHistoryChunks.count_by_job_id_and_status(job.id, [status])
        assign(socket, :"total_#{status}", total)
      end
    )
    |> assign(total_chunks: OpenInterestHistoryChunks.count_by_job_id(job.id))
  end
end
