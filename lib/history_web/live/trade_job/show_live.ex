defmodule HistoryWeb.TradeJob.ShowLive do
  ...
  def mount(%{"id" => id}, _session, socket) do
    ...
    socket =
      socket
      |> assign(job: TradeHistoryJobs.get!(id))
  ...
  def handle_info({:trade_history_job, :update, _}, socket) do
    socket =
      socket
      |> assign(job: TradeHistoryJobs.get!(socket.assigns.job.id))
      |> assign_chunk_status_totals()
      |> assign_chunks()
  ...
  @impl true
  def handle_info({:trade_history_chunk, :update, _}, socket) do
    socket =
      socket
      |> assign_chunk_status_totals()
      |> assign_chunks()
  ...
  defp assign_chunks(socket) do
    ...
    |> assign(
      :chunks,
      TradeHistoryChunks.by_job(job.id,
        page: current_page,
  ...
  defp assign_chunk_status_totals(socket) do
    ...
    |> Enum.reduce(
      ...
      fn {status, _color}, socket ->
        total = TradeHistoryChunks.count_by_job_id_and_status(job.id, status)
        assign(socket, :"total_#{status}", total)
      end
    )
    |> assign(total_chunks: TradeHistoryChunks.count_by_job_id(job.id))
  end
end
