defmodule Ghost.LendingRateHistoryJobs.DownloadChunksBroadway do
  use Broadway
  require Logger
  alias Broadway.Message
  alias Ghost.LendingRateHistoryChunks
  alias Ghost.LendingRateHistoryJobs
  alias Ghost.LendingRates
  ...
  def ack(:ack_id, successful, failed) do
    ...
    |> Enum.each(fn m ->
      Logger.error(
        "could not download chunk - id: #{m.data.id}, venue: #{m.data.venue}, token: #{
          m.data.token
        }"
      )
